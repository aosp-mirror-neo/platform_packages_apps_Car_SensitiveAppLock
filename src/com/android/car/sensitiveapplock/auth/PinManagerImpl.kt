...
constructor(
    ...
    private val encryptedKeysetStringFlow = sharedPreferences.data.map {
        it[TINK_KEYSET_NAME] ?: ""
    }

    override suspend fun getAppLockPinState(): PinState = withContext(backgroundContext) {
    ...
    override suspend fun setAppLockPin(pin: String): Boolean = withContext(backgroundContext) {
        ...
        try {
            aead?.encrypt(
                ...
                appLockDataRepository.setPin(Hex.encode(it))
            }
    ...
    override suspend fun clearAppLockPin() = withContext(backgroundContext) {
        appLockDataRepository.setPin("")
    }

    override suspend fun verifyAppLockPin(pin: String): Boolean = withContext(backgroundContext) {
    ...
    private suspend fun maybeInitializeKeysetHandleAndAeadKeyset(): Boolean {
        try {
            ...
            val keysetHandle = TinkProtoKeysetFormat.parseEncryptedKeyset(
                encryptedKeyset,
